...
class PixFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel =
            ViewModelProvider(this,
                ViewModelProvider.NewInstanceFactory()).get(PixViewModel::class.java)

        val root = inflater.inflate (R.layout.fragment_pix, container, false)

        viewModel.getMethodsPix()
        initView()
    ...
    private fun initView(){
        viewModel.listMethods.observe(viewLifecycleOwner){
            recyclerPix.layoutManager = LinearLayoutManager(context)
            val adapter = MethodsAdapter(it!!, this::onItemClick)
            recyclerPix.adapter = adapter
        }
    }
    ...
    private fun onItemClick(method: Methods) {
        ...
        findNavController().navigate(R.id.action_pixFragment_to_pixFormFragment, bundle)
    }
}